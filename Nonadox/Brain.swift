...
//  Nonadox
//
//  Created by Justin Li on 4/16/17.
...
class Brain{
    ...
    init() {
        ...
        buttons = [SKSpriteNode(imageNamed:"menu"),SKSpriteNode(imageNamed:"restart")]
        moves = 0
        
    }
    func setLocation(){
        
        locations = [CGPoint(x: scene.size.width * 0.5, y: scene.size.height * 0.605),
                     CGPoint(x: scene.size.width * 0.64, y: scene.size.height * 0.525),
                     CGPoint(x: scene.size.width * 0.77, y: scene.size.height * 0.45),
                     CGPoint(x: scene.size.width * 0.36, y: scene.size.height * 0.525),
                     CGPoint(x: scene.size.width * 0.5, y: scene.size.height * 0.45),
                     CGPoint(x: scene.size.width * 0.64, y: scene.size.height * 0.375),
                     CGPoint(x: scene.size.width * 0.23, y: scene.size.height * 0.45),
                     CGPoint(x: scene.size.width * 0.36, y: scene.size.height * 0.375),
                     CGPoint(x: scene.size.width * 0.5, y: scene.size.height * 0.295)]
    ...
    func winningCondition(winState: [Int]){
        ...
        for i in 0..<9 {
            ...
            diamonds[i].size = CGSize(width: 120, height: 120)
    ...
    func levelNumber(levelNum: Int, nextNum: Int) {
        ...
        stages.text = ("S T A G E  " + "\(levelNum)")
        stages.fontSize = 50
            ...
            for j in 0..<self.direct[i].count{
                if j == 0{
                    if self.direct[i][j] == 1{
                        ...
                        direction.size = CGSize(width: 120, height: 120)
                    ...
                    if self.direct[i][j] == 1{
                        ...
                        direction.size = CGSize(width: 120, height: 120)
                        direction.zPosition = 3
                        direction.zRotation = CGFloat(Double.pi) * 1.5
                        scene.addChild(direction)
                    }
                    if self.direct[i][j] == 2{
                        let direction = SKSpriteNode(imageNamed:"double")
                    ...
                    if self.direct[i][j] == 1{
                        ...
                        direction.size = CGSize(width: 120, height: 120)
                        direction.zPosition = 3
                        direction.zRotation = CGFloat(Double.pi)
                        scene.addChild(direction)
                    }
                    if self.direct[i][j] == 2{
                        let direction = SKSpriteNode(imageNamed:"double")
                    ...
                    if self.direct[i][j] == 1{
                        ...
                        direction.size = CGSize(width: 120, height: 120)
                        direction.zPosition = 3
                        direction.zRotation = CGFloat(Double.pi) * 0.5
                        scene.addChild(direction)
                    }
                    if self.direct[i][j] == 2{
                        let direction = SKSpriteNode(imageNamed:"double")
    ...
    func load() {
        ...
        for i in 0..<9 {
            ...
            innerDiamonds[i].size = CGSize(width: 120, height: 120)
    ...
    func animate(pointTwo: Int, angle: Int){
        ...
        case 0:
            ...
            SystemSoundID.playFileNamed("e", withExtenstion: "wav")
            break
        case 1:
            ...
            SystemSoundID.playFileNamed("g#", withExtenstion: "wav")
            break
        case 2:
            innerDiamonds[pointTwo].run(SKAction.sequence([SKAction.rotate(byAngle: CGFloat(Angle), duration: 0),AnimationThree,SKAction.rotate(byAngle: -CGFloat(Angle), duration: 0)]))
             SystemSoundID.playFileNamed("b", withExtenstion: "wav")
            break
    ...
    func setScore(gold: Int, silver: Int, bronze: Int){
        ...
        goldText.position = CGPoint(x: scene.size.width * 0.42, y: scene.size.height * 0.09)
        goldText.fontSize = 35
        goldText.text = ("\(gold)")
        ...
        silverText.position = CGPoint(x: scene.size.width * 0.28, y: scene.size.height * 0.09)
        silverText.fontSize = 35
        ...
        bronzeText.position = CGPoint(x: scene.size.width * 0.14, y: scene.size.height * 0.09)
        bronzeText.fontSize = 35
